...
#include "simde/simde/x86/sse.h" /* Float conversion in division-based blends */
#include "simde/simde/x86/sse2.h"
#include "simde/simde/x86/sse4.1.h" /* _mm_max_epi32, _mm_min_epi32 for clamping */
...
static inline __m128i simd_composite_with_alpha(__m128i blend_result, __m128i src, __m128i dst) {
    ...
    /* Use 255 for alpha in blend_result so output alpha follows Porter-Duff:
     * out_a = src_a + dst_a * (255 - src_a) / 255. Blend modes produce RGB only;
     * blending alpha via the mode formula would yield wrong output alpha. */
    blend_result = _mm_or_si128(blend_result, _mm_set1_epi32(0xFF000000));
    ...
    /* Add: blend_scaled + dst_scaled, clamp to [0,255] to match scalar path */
    ...
    result_lo = _mm_max_epi16(_mm_min_epi16(result_lo, max255), zero);
    result_hi = _mm_max_epi16(_mm_min_epi16(result_hi, max255), zero);
 ...
 * Formula: out = src + dst * (255 - src_alpha) / 255, clamped to [0,255]
 */
static inline __m128i simd_blend_over(__m128i src, __m128i dst) {
    ...
    /* out = src + dst_scaled in 16-bit, then clamp to [0,255] to match scalar path */
    __m128i result_lo = _mm_add_epi16(src_lo, dst_scaled_lo);
    __m128i result_hi = _mm_add_epi16(src_hi, dst_scaled_hi);
    result_lo = _mm_max_epi16(_mm_min_epi16(result_lo, max255), zero);
    result_hi = _mm_max_epi16(_mm_min_epi16(result_hi, max255), zero);
...
static inline __m128i simd_blend_screen(__m128i src, __m128i dst) {
    ...
    /* blend = src + dst - product (can underflow in 16-bit) */
    ...
    /* Clamp to [0,255] before pack - avoids edge artifacts from 16-bit wraparound */
    __m128i max255 = _mm_set1_epi16(255);
    blend_lo = _mm_max_epi16(_mm_min_epi16(blend_lo, max255), zero);
    blend_hi = _mm_max_epi16(_mm_min_epi16(blend_hi, max255), zero);
...
/**
 * Color Burn: blend = 255 - min(255, (255-dst)*255/src), src==0 -> 0
 * Uses float division for exact scalar match.
 */
static inline __m128i simd_blend_color_burn(__m128i src, __m128i dst) {
    __m128i zero_i = _mm_setzero_si128();
    __m128i r_s = _mm_and_si128(_mm_srli_epi32(src, 16), _mm_set1_epi32(0xFF));
    __m128i g_s = _mm_and_si128(_mm_srli_epi32(src, 8), _mm_set1_epi32(0xFF));
    __m128i b_s = _mm_and_si128(src, _mm_set1_epi32(0xFF));
    __m128i r_d = _mm_and_si128(_mm_srli_epi32(dst, 16), _mm_set1_epi32(0xFF));
    __m128i g_d = _mm_and_si128(_mm_srli_epi32(dst, 8), _mm_set1_epi32(0xFF));
    __m128i b_d = _mm_and_si128(dst, _mm_set1_epi32(0xFF));
    __m128 one_255 = _mm_set1_ps(255.0f);
    __m128 eps = _mm_set1_ps(1.0f);
    __m128 rF = _mm_cvtepi32_ps(r_s);
    __m128 gF = _mm_cvtepi32_ps(g_s);
    __m128 bF = _mm_cvtepi32_ps(b_s);
    __m128 rD = _mm_cvtepi32_ps(r_d);
    __m128 gD = _mm_cvtepi32_ps(g_d);
    __m128 bD = _mm_cvtepi32_ps(b_d);
    __m128 inv_r = _mm_sub_ps(one_255, rD);
    __m128 inv_g = _mm_sub_ps(one_255, gD);
    __m128 inv_b = _mm_sub_ps(one_255, bD);
    __m128 div_r = _mm_div_ps(_mm_mul_ps(inv_r, one_255), _mm_max_ps(rF, eps));
    __m128 div_g = _mm_div_ps(_mm_mul_ps(inv_g, one_255), _mm_max_ps(gF, eps));
    __m128 div_b = _mm_div_ps(_mm_mul_ps(inv_b, one_255), _mm_max_ps(bF, eps));
    __m128 burn_r = _mm_sub_ps(one_255, _mm_min_ps(div_r, one_255));
    __m128 burn_g = _mm_sub_ps(one_255, _mm_min_ps(div_g, one_255));
    __m128 burn_b = _mm_sub_ps(one_255, _mm_min_ps(div_b, one_255));
    __m128i ri = _mm_cvtps_epi32(burn_r);
    __m128i gi = _mm_cvtps_epi32(burn_g);
    __m128i bi = _mm_cvtps_epi32(burn_b);
    ri = _mm_max_epi32(_mm_min_epi32(ri, _mm_set1_epi32(255)), zero_i);
    gi = _mm_max_epi32(_mm_min_epi32(gi, _mm_set1_epi32(255)), zero_i);
    bi = _mm_max_epi32(_mm_min_epi32(bi, _mm_set1_epi32(255)), zero_i);
    __m128i r_zero = _mm_cmpeq_epi32(r_s, zero_i);
    __m128i g_zero = _mm_cmpeq_epi32(g_s, zero_i);
    __m128i b_zero = _mm_cmpeq_epi32(b_s, zero_i);
    ri = _mm_andnot_si128(r_zero, ri);
    gi = _mm_andnot_si128(g_zero, gi);
    bi = _mm_andnot_si128(b_zero, bi);
    __m128i blend_result = _mm_or_si128(_mm_or_si128(_mm_slli_epi32(ri, 16),
                                                     _mm_slli_epi32(gi, 8)),
                                        bi);
    blend_result = _mm_or_si128(blend_result, _mm_and_si128(src, _mm_set1_epi32(0xFF000000)));
    return simd_composite_with_alpha(blend_result, src, dst);
}

/**
 * Color Dodge: blend = min(255, dst*255/(255-src)), src==255 -> 255
 * Uses float division for exact scalar match.
 */
static inline __m128i simd_blend_color_dodge(__m128i src, __m128i dst) {
    ...
    __m128 gD = _mm_cvtepi32_ps(g_d);
    __m128 bD = _mm_cvtepi32_ps(b_d);
    __m128 denom_r = _mm_sub_ps(one_255, rF);
    __m128 denom_g = _mm_sub_ps(one_255, gF);
    __m128 denom_b = _mm_sub_ps(one_255, bF);
    denom_r = _mm_max_ps(denom_r, _mm_set1_ps(1.0f));
    denom_g = _mm_max_ps(denom_g, _mm_set1_ps(1.0f));
    denom_b = _mm_max_ps(denom_b, _mm_set1_ps(1.0f));
    __m128 dodge_r = _mm_min_ps(_mm_div_ps(_mm_mul_ps(rD, one_255), denom_r), one_255);
    __m128 dodge_g = _mm_min_ps(_mm_div_ps(_mm_mul_ps(gD, one_255), denom_g), one_255);
    __m128 dodge_b = _mm_min_ps(_mm_div_ps(_mm_mul_ps(bD, one_255), denom_b), one_255);
    __m128i ri = _mm_max_epi32(_mm_min_epi32(_mm_cvtps_epi32(dodge_r), _mm_set1_epi32(255)), zero_i);
    __m128i gi = _mm_max_epi32(_mm_min_epi32(_mm_cvtps_epi32(dodge_g), _mm_set1_epi32(255)), zero_i);
    __m128i bi = _mm_max_epi32(_mm_min_epi32(_mm_cvtps_epi32(dodge_b), _mm_set1_epi32(255)), zero_i);
    __m128i r_max = _mm_cmpeq_epi32(r_s, _mm_set1_epi32(255));
    __m128i g_max = _mm_cmpeq_epi32(g_s, _mm_set1_epi32(255));
    __m128i b_max = _mm_cmpeq_epi32(b_s, _mm_set1_epi32(255));
    ri = _mm_or_si128(_mm_and_si128(r_max, _mm_set1_epi32(255)), _mm_andnot_si128(r_max, ri));
    gi = _mm_or_si128(_mm_and_si128(g_max, _mm_set1_epi32(255)), _mm_andnot_si128(g_max, gi));
    bi = _mm_or_si128(_mm_and_si128(b_max, _mm_set1_epi32(255)), _mm_andnot_si128(b_max, bi));
    __m128i blend_result = _mm_or_si128(_mm_or_si128(_mm_slli_epi32(ri, 16),
                                                     _mm_slli_epi32(gi, 8)),
                                        bi);
    blend_result = _mm_or_si128(blend_result, _mm_and_si128(src, _mm_set1_epi32(0xFF000000)));
...
/**
 * Extract R, G, B from 4 ARGB pixels for luminance (Cairo ARGB32: B,G,R,A in memory)
 */
static inline void simd_extract_rgb_4px(__m128i pixels,
                                        __m128i* r_16, __m128i* g_16, __m128i* b_16) {
    __m128i zero = _mm_setzero_si128();
    /* R at byte 2: _mm_srli_epi32(p,16) & 0xFF */
    __m128i r_32 = _mm_and_si128(_mm_srli_epi32(pixels, 16), _mm_set1_epi32(0xFF));
    __m128i g_32 = _mm_and_si128(_mm_srli_epi32(pixels, 8), _mm_set1_epi32(0xFF));
    __m128i b_32 = _mm_and_si128(pixels, _mm_set1_epi32(0xFF));
    /* Pack 4x32 to 8x16 (lower 4 from each) */
    *r_16 = _mm_packs_epi32(r_32, zero);
    *g_16 = _mm_packs_epi32(g_32, zero);
    *b_16 = _mm_packs_epi32(b_32, zero);
}

/**
 * SIMD Darker Color blend - pick whole pixel with lower luminance
 */
static inline __m128i simd_blend_darker_color(__m128i src, __m128i dst) {
    __m128i src_r, src_g, src_b, dst_r, dst_g, dst_b;
    simd_extract_rgb_4px(src, &src_r, &src_g, &src_b);
    simd_extract_rgb_4px(dst, &dst_r, &dst_g, &dst_b);
    __m128i src_lum = simd_calc_luminance_16(src_r, src_g, src_b);
    __m128i dst_lum = simd_calc_luminance_16(dst_r, dst_g, dst_b);
    __m128i mask_16 = _mm_cmplt_epi16(src_lum, dst_lum); /* 0xFFFF where src darker */
    __m128i mask_16x = _mm_unpacklo_epi16(mask_16, mask_16);
    __m128i mask_32 = _mm_cmpeq_epi32(mask_16x, _mm_set1_epi32(-1)); /* 0xFFFFFFFF */
    __m128i blend_result = _mm_or_si128(_mm_and_si128(mask_32, src),
                                        _mm_andnot_si128(mask_32, dst));
    return simd_composite_with_alpha(blend_result, src, dst);
}
...
/**
 * SIMD Lighter Color blend - pick whole pixel with higher luminance
 */
static inline __m128i simd_blend_lighter_color(__m128i src, __m128i dst) {
    ...
    __m128i src_lum = simd_calc_luminance_16(src_r, src_g, src_b);
    __m128i dst_lum = simd_calc_luminance_16(dst_r, dst_g, dst_b);
    __m128i mask_16 = _mm_cmpgt_epi16(src_lum, dst_lum); /* 0xFFFF where src lighter */
    __m128i mask_16x = _mm_unpacklo_epi16(mask_16, mask_16);
    __m128i mask_32 = _mm_cmpeq_epi32(mask_16x, _mm_set1_epi32(-1)); /* 0xFFFFFFFF */
    __m128i blend_result = _mm_or_si128(_mm_and_si128(mask_32, src),
                                        _mm_andnot_si128(mask_32, dst));
...
/**
 * Vivid Light: src<128: Color Burn with 2*src divisor; src>=128: Color Dodge with 2*(255-src)
 */
static inline __m128i simd_blend_vivid_light(__m128i src, __m128i dst) {
    ...
    __m128 inv_g = _mm_sub_ps(one_255, gD);
    __m128 inv_b = _mm_sub_ps(one_255, bD);
    __m128 denom_burn_r = _mm_mul_ps(two, _mm_max_ps(rF, eps));
    __m128 denom_burn_g = _mm_mul_ps(two, _mm_max_ps(gF, eps));
    __m128 denom_burn_b = _mm_mul_ps(two, _mm_max_ps(bF, eps));
    __m128 denom_dodge_r = _mm_mul_ps(two, _mm_max_ps(_mm_sub_ps(one_255, rF), eps));
    __m128 denom_dodge_g = _mm_mul_ps(two, _mm_max_ps(_mm_sub_ps(one_255, gF), eps));
    __m128 denom_dodge_b = _mm_mul_ps(two, _mm_max_ps(_mm_sub_ps(one_255, bF), eps));
    __m128 burn_r = _mm_sub_ps(one_255, _mm_min_ps(_mm_div_ps(_mm_mul_ps(inv_r, one_255), denom_burn_r), one_255));
    __m128 burn_g = _mm_sub_ps(one_255, _mm_min_ps(_mm_div_ps(_mm_mul_ps(inv_g, one_255), denom_burn_g), one_255));
    __m128 burn_b = _mm_sub_ps(one_255, _mm_min_ps(_mm_div_ps(_mm_mul_ps(inv_b, one_255), denom_burn_b), one_255));
    __m128 dodge_r = _mm_min_ps(_mm_div_ps(_mm_mul_ps(rD, one_255), denom_dodge_r), one_255);
    __m128 dodge_g = _mm_min_ps(_mm_div_ps(_mm_mul_ps(gD, one_255), denom_dodge_g), one_255);
    __m128 dodge_b = _mm_min_ps(_mm_div_ps(_mm_mul_ps(bD, one_255), denom_dodge_b), one_255);
    __m128 mask_r = _mm_cmplt_ps(rF, _mm_set1_ps(128.0f));
    __m128 mask_g = _mm_cmplt_ps(gF, _mm_set1_ps(128.0f));
    __m128 mask_b = _mm_cmplt_ps(bF, _mm_set1_ps(128.0f));
    __m128 res_r = _mm_or_ps(_mm_and_ps(mask_r, burn_r), _mm_andnot_ps(mask_r, dodge_r));
    __m128 res_g = _mm_or_ps(_mm_and_ps(mask_g, burn_g), _mm_andnot_ps(mask_g, dodge_g));
    __m128 res_b = _mm_or_ps(_mm_and_ps(mask_b, burn_b), _mm_andnot_ps(mask_b, dodge_b));
    __m128i ri = _mm_max_epi32(_mm_min_epi32(_mm_cvtps_epi32(res_r), _mm_set1_epi32(255)), zero_i);
    __m128i gi = _mm_max_epi32(_mm_min_epi32(_mm_cvtps_epi32(res_g), _mm_set1_epi32(255)), zero_i);
    __m128i bi = _mm_max_epi32(_mm_min_epi32(_mm_cvtps_epi32(res_b), _mm_set1_epi32(255)), zero_i);
    __m128i r_zero = _mm_cmpeq_epi32(r_s, zero_i);
    __m128i g_zero = _mm_cmpeq_epi32(g_s, zero_i);
    __m128i b_zero = _mm_cmpeq_epi32(b_s, zero_i);
    ri = _mm_andnot_si128(r_zero, ri);
 ...
 * Use (src*dst + 64) >> 7 instead of (2*src*dst + 128) >> 8 to avoid 16-bit overflow
 */
static inline __m128i simd_blend_exclusion(__m128i src, __m128i dst) {
    __m128i zero = _mm_setzero_si128();
    __m128i round64 = _mm_set1_epi16(64);
    ...
    /* 2*src*dst/255 = (src*dst + 64) >> 7 (avoids 16-bit overflow from 2*255*255) */
    __m128i prod_lo = _mm_mullo_epi16(src_lo, dst_lo);
    __m128i prod_hi = _mm_mullo_epi16(src_hi, dst_hi);
    prod_lo = _mm_add_epi16(prod_lo, round64);
    prod_hi = _mm_add_epi16(prod_hi, round64);
    prod_lo = _mm_srli_epi16(prod_lo, 7);
    prod_hi = _mm_srli_epi16(prod_hi, 7);

    /* src + dst - 2*product (can underflow in 16-bit) */
    __m128i blend_lo = _mm_add_epi16(src_lo, dst_lo);
    __m128i blend_hi = _mm_add_epi16(src_hi, dst_hi);
    blend_lo = _mm_sub_epi16(blend_lo, prod_lo);
    blend_hi = _mm_sub_epi16(blend_hi, prod_hi);

    /* Clamp to [0,255] before pack - avoids edge artifacts from wraparound */
    __m128i max255 = _mm_set1_epi16(255);
    blend_lo = _mm_max_epi16(_mm_min_epi16(blend_lo, max255), zero);
    blend_hi = _mm_max_epi16(_mm_min_epi16(blend_hi, max255), zero);
...
/**
 * Divide: blend = min(255, dst*255/src) if src > 0, else 255
 */
static inline __m128i simd_blend_divide(__m128i src, __m128i dst) {
    ...
    __m128 one_255 = _mm_set1_ps(255.0f);
    __m128 eps = _mm_set1_ps(1.0f);
    __m128 rF = _mm_cvtepi32_ps(_mm_max_epi32(r_s, _mm_set1_epi32(1)));
    __m128 gF = _mm_cvtepi32_ps(_mm_max_epi32(g_s, _mm_set1_epi32(1)));
    __m128 bF = _mm_cvtepi32_ps(_mm_max_epi32(b_s, _mm_set1_epi32(1)));
    __m128 rD = _mm_cvtepi32_ps(r_d);
    __m128 gD = _mm_cvtepi32_ps(g_d);
    __m128 bD = _mm_cvtepi32_ps(b_d);
    __m128 div_r = _mm_min_ps(_mm_div_ps(_mm_mul_ps(rD, one_255), rF), one_255);
    __m128 div_g = _mm_min_ps(_mm_div_ps(_mm_mul_ps(gD, one_255), gF), one_255);
    __m128 div_b = _mm_min_ps(_mm_div_ps(_mm_mul_ps(bD, one_255), bF), one_255);
    __m128i ri = _mm_max_epi32(_mm_min_epi32(_mm_cvtps_epi32(div_r), _mm_set1_epi32(255)), zero_i);
    __m128i gi = _mm_max_epi32(_mm_min_epi32(_mm_cvtps_epi32(div_g), _mm_set1_epi32(255)), zero_i);
    __m128i bi = _mm_max_epi32(_mm_min_epi32(_mm_cvtps_epi32(div_b), _mm_set1_epi32(255)), zero_i);
    __m128i r_zero = _mm_cmpeq_epi32(r_s, zero_i);
    __m128i g_zero = _mm_cmpeq_epi32(g_s, zero_i);
    __m128i b_zero = _mm_cmpeq_epi32(b_s, zero_i);
    ri = _mm_or_si128(_mm_and_si128(r_zero, _mm_set1_epi32(255)), _mm_andnot_si128(r_zero, ri));
    gi = _mm_or_si128(_mm_and_si128(g_zero, _mm_set1_epi32(255)), _mm_andnot_si128(g_zero, gi));
    bi = _mm_or_si128(_mm_and_si128(b_zero, _mm_set1_epi32(255)), _mm_andnot_si128(b_zero, bi));
    __m128i blend_result = _mm_or_si128(_mm_or_si128(_mm_slli_epi32(ri, 16),
                                                     _mm_slli_epi32(gi, 8)),
                                        bi);
    blend_result = _mm_or_si128(blend_result, _mm_and_si128(src, _mm_set1_epi32(0xFF000000)));
...
static inline guint32 scalar_composite_with_alpha(guint8 blend_r, guint8 blend_g, guint8 blend_b,
    ...
    gint32 out_r = (((guint32)blend_r * src_a + (guint32)dst_r * inv_src_a + 128) >> 8);
    gint32 out_g = (((guint32)blend_g * src_a + (guint32)dst_g * inv_src_a + 128) >> 8);
    gint32 out_b = (((guint32)blend_b * src_a + (guint32)dst_b * inv_src_a + 128) >> 8);
    gint32 out_a = (gint32)src_a + (((guint32)dst_a * inv_src_a + 128) >> 8);
    out_r = CLAMP(out_r, 0, 255);
    out_g = CLAMP(out_g, 0, 255);
    out_b = CLAMP(out_b, 0, 255);
    out_a = CLAMP(out_a, 0, 255);

    return ((guint8)out_a << 24) | ((guint8)out_r << 16) | ((guint8)out_g << 8) | (guint8)out_b;
}
...
static inline guint32 scalar_blend_screen(guint32 src_pixel, guint32 dst_pixel) {
    ...
    gint32 blend_r = (gint32)src_r + dst_r - ((src_r * dst_r + 128) >> 8);
    gint32 blend_g = (gint32)src_g + dst_g - ((src_g * dst_g + 128) >> 8);
    gint32 blend_b = (gint32)src_b + dst_b - ((src_b * dst_b + 128) >> 8);
...
static inline guint32 scalar_blend_overlay(guint32 src_pixel, guint32 dst_pixel) {
    ...
    guint8 blend_r = (dst_r < 128) ? (guint8)((2 * (guint32)src_r * dst_r + 128) >> 8) : (guint8)(255 - ((2 * (guint32)(255 - src_r) * (255 - dst_r) + 128) >> 8));
    guint8 blend_g = (dst_g < 128) ? (guint8)((2 * (guint32)src_g * dst_g + 128) >> 8) : (guint8)(255 - ((2 * (guint32)(255 - src_g) * (255 - dst_g) + 128) >> 8));
    guint8 blend_b = (dst_b < 128) ? (guint8)((2 * (guint32)src_b * dst_b + 128) >> 8) : (guint8)(255 - ((2 * (guint32)(255 - src_b) * (255 - dst_b) + 128) >> 8));
...
static inline guint32 scalar_blend_hard_light(guint32 src_pixel, guint32 dst_pixel) {
    ...
    guint8 blend_r = (src_r < 128) ? (guint8)((2 * (guint32)src_r * dst_r + 128) >> 8) : (guint8)(255 - ((2 * (guint32)(255 - src_r) * (255 - dst_r) + 128) >> 8));
    guint8 blend_g = (src_g < 128) ? (guint8)((2 * (guint32)src_g * dst_g + 128) >> 8) : (guint8)(255 - ((2 * (guint32)(255 - src_g) * (255 - dst_g) + 128) >> 8));
    guint8 blend_b = (src_b < 128) ? (guint8)((2 * (guint32)src_b * dst_b + 128) >> 8) : (guint8)(255 - ((2 * (guint32)(255 - src_b) * (255 - dst_b) + 128) >> 8));
...
static inline guint32 scalar_blend_over(guint32 src_pixel, guint32 dst_pixel, guint8 layer_opacity) {
    ...
    guint8 inv_src_a = 255 - src_a;
    gint32 out_a = (gint32)src_a + (((guint32)dst_a * inv_src_a + 128) >> 8);
    gint32 out_r = (gint32)src_r + (((guint32)dst_r * inv_src_a + 128) >> 8);
    gint32 out_g = (gint32)src_g + (((guint32)dst_g * inv_src_a + 128) >> 8);
    gint32 out_b = (gint32)src_b + (((guint32)dst_b * inv_src_a + 128) >> 8);
    out_r = CLAMP(out_r, 0, 255);
    out_g = CLAMP(out_g, 0, 255);
    out_b = CLAMP(out_b, 0, 255);
    out_a = CLAMP(out_a, 0, 255);
...
static inline guint32 scalar_blend_vivid_light(guint32 src_pixel, guint32 dst_pixel) {
    ...
    guint8 blend_r = (src_r < 128) ? ((src_r == 0) ? 0 : (guint8)(255 - MIN(255, ((255 - dst_r) * 255) / (2 * src_r)))) : ((src_r == 255) ? 255 : (guint8)MIN(255, (dst_r * 255) / (2 * (255 - src_r))));
    guint8 blend_g = (src_g < 128) ? ((src_g == 0) ? 0 : (guint8)(255 - MIN(255, ((255 - dst_g) * 255) / (2 * src_g)))) : ((src_g == 255) ? 255 : (guint8)MIN(255, (dst_g * 255) / (2 * (255 - src_g))));
    guint8 blend_b = (src_b < 128) ? ((src_b == 0) ? 0 : (guint8)(255 - MIN(255, ((255 - dst_b) * 255) / (2 * src_b)))) : ((src_b == 255) ? 255 : (guint8)MIN(255, (dst_b * 255) / (2 * (255 - src_b))));
...
static inline guint32 scalar_blend_exclusion(guint32 src_pixel, guint32 dst_pixel) {
    ...
    /* src + dst - 2*src*dst/255 (can overflow/underflow 8-bit) */
    gint32 blend_r = (gint32)src_r + dst_r - ((2 * (guint32)src_r * dst_r + 128) >> 8);
    gint32 blend_g = (gint32)src_g + dst_g - ((2 * (guint32)src_g * dst_g + 128) >> 8);
    gint32 blend_b = (gint32)src_b + dst_b - ((2 * (guint32)src_b * dst_b + 128) >> 8);
    blend_r = CLAMP(blend_r, 0, 255);
    blend_g = CLAMP(blend_g, 0, 255);
    blend_b = CLAMP(blend_b, 0, 255);
...
static inline void rgb_to_hsl(guint8 r, guint8 g, guint8 b,
    ...
    *l = (gint32)CLAMP((max_val + min_val) / 2, 0, 255);

    if (delta == 0) {
        *h = 0;
        *s = 0;
    } else {
        gint32 s_val = (*l < 128) ? (delta * 255) / (max_val + min_val) : (delta * 255) / (510 - max_val - min_val);
        *s = (gint32)CLAMP(s_val, 0, 255);
...
static inline guint8 hsl_value(gint32 n1, gint32 n2, gint32 hue) {
    ...
    if (hue < 60)
        ...
        v = n1;
    return (guint8)CLAMP(v, 0, 255);
}

static inline void hsl_to_rgb(gint32 h, gint32 s, gint32 l,
                              guint8* r, guint8* g, guint8* b) {
    if (s == 0) {
        *r = *g = *b = (guint8)CLAMP(l, 0, 255);
    } else {
        gint32 m2 = (l < 128) ? (l * (255 + s)) / 255 : l + s - (l * s) / 255;
        gint32 m1 = 2 * l - m2;
        m1 = CLAMP(m1, 0, 255);
        m2 = CLAMP(m2, 0, 255);
...
void blend_composite_row_over(const guint32* src_row, guint32* dst_row,
    ...
    for (; x < width; x++) {
        ...
        dst_row[x] = ((guint8)out_a << 24) | ((guint8)out_r << 16) | ((guint8)out_g << 8) | (guint8)out_b;
    }
}
