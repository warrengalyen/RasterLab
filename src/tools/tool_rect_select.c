...
void tool_rect_select_draw_preview(ImageDocument* doc, cairo_t* cr, gdouble zoom) {
    ...
    if (doc->layers && g_list_length(doc->layers) > 0) {
        ...
        if (show_feathered) {
             ...
             * Building the bounded SelectionMask triggers an O(selection_area) EDT.
             * When only the animation phase changes (every timer tick) the bounds and
             * feather parameters are identical, so we keep the computed mask alive in
             * state->preview_cache and skip the EDT entirely on cache hits.
             * The cache is invalidated (and freed) whenever bounds, feather radius,
             * smoothing mode, or combine mode change.  It is freed in reset(). */
            ...
            if (!cache_valid) {
                ...
                /* Build bounded preview mask (O(selection_area) EDT). */
                ...
                if (preview_sel) {
                    ...
                    /* Trigger feather computation now so it is already cached for rendering */
            ...
            /* Render from cache (EDT already done; only the outline scan runs here) */
            if (state->preview_cache) {
