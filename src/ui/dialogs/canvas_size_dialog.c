...
CanvasSizeDialog* canvas_size_dialog_new(ImageDocument* doc) {
    ...
    /* Get all widgets */
    {
        GtkWidget* wph = GTK_WIDGET(gtk_builder_get_object(builder, "canvas_size_width_spin_placeholder"));
        GtkWidget* hph = GTK_WIDGET(gtk_builder_get_object(builder, "canvas_size_height_spin_placeholder"));
        GtkWidget* rph = GTK_WIDGET(gtk_builder_get_object(builder, "canvas_size_resolution_spin_placeholder"));
        ...
        dialog->width_spin = wph ? vertical_spin_button_new(width_adj, 1.0, 2) : NULL;
        dialog->height_spin = hph ? vertical_spin_button_new(height_adj, 1.0, 2) : NULL;
        dialog->resolution_spin = rph ? vertical_spin_button_new(resolution_adj, 1.0, 2) : NULL;
        if (dialog->width_spin && wph) {
            gtk_box_pack_start(GTK_BOX(wph), dialog->width_spin, FALSE, TRUE, 0);
        }
        if (dialog->height_spin && hph) {
            gtk_box_pack_start(GTK_BOX(hph), dialog->height_spin, FALSE, TRUE, 0);
        }
        if (dialog->resolution_spin && rph) {
            gtk_box_pack_start(GTK_BOX(rph), dialog->resolution_spin, FALSE, TRUE, 0);
        }
    }
