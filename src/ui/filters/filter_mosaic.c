...
gboolean filter_mosaic_apply(ImageLayer* layer, const gfloat* values, gint num_values) {
    ...
    if (!rgb_input || !rgb_output) {
        g_warning("Mosaic filter: Failed to allocate memory");
        g_free(rgb_input);
        g_free(rgb_output);
        return FALSE;
    }
    ...
    if (!adjustments_cairo_to_rgb(surface, rgb_input)) {
        g_warning("Mosaic filter: Failed to convert surface to RGB");
        g_free(rgb_input);
        g_free(rgb_output);
        return FALSE;
    }
    ...
    if (status != OC_STATUS_OK) {
        g_warning("Mosaic filter: Ocular filter returned error %d", status);
        g_free(rgb_input);
        g_free(rgb_output);
        return FALSE;
    }
    ...
    if (!adjustments_rgb_to_cairo(surface, rgb_output)) {
        g_warning("Mosaic filter: Failed to convert RGB to surface");
        g_free(rgb_input);
        g_free(rgb_output);
        return FALSE;
    }
