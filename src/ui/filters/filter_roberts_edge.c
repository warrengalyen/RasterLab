...
gboolean filter_roberts_edge_apply(ImageLayer* layer) {
    ...
    if (!rgb_input || !grayscale_input || !edge_output) {
        debug_log("WRN", "Roberts edge filter: Failed to allocate memory");
        g_free(rgb_input);
    ...
    if (!adjustments_cairo_to_rgb(surface, rgb_input)) {
        debug_log("WRN", "Roberts edge filter: Failed to convert surface to RGB");
        g_free(rgb_input);
        g_free(grayscale_input);
        g_free(edge_output);
        return FALSE;
    ...
    if (status != OC_STATUS_OK) {
        debug_log("WRN", "Roberts edge filter: Grayscale conversion returned error %d", status);
        g_free(rgb_input);
        g_free(grayscale_input);
        g_free(edge_output);
        return FALSE;
    ...
    /* Apply Roberts edge filter using Ocular library
       Input and output: Single channel grayscale (Channels = 1) */
    status = ocularRobertsEdgeDetect(grayscale_input, edge_output, width, height, 1);

    if (status != OC_STATUS_OK) {
        debug_log("WRN", "Roberts edge filter: Ocular filter returned error %d", status);
        g_free(rgb_input);
        g_free(grayscale_input);
        g_free(edge_output);
        return FALSE;
    ...
    if (!adjustments_grayscale_to_cairo(surface, edge_output)) {
        debug_log("WRN", "Roberts edge filter: Failed to convert grayscale to surface");
        g_free(rgb_input);
        g_free(grayscale_input);
        g_free(edge_output);
        return FALSE;
