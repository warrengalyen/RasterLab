...
gboolean filter_vibrance_apply(ImageLayer* layer, const gfloat* values, gint num_values) {
    ...
    if (!filter_utils_allocate_rgb_buffers(layer->surface, &buffers, "Vibrance filter")) {
        return FALSE;
    }

    /* Convert from Cairo ARGB32 to RGB */
    if (!filter_utils_cairo_to_rgb(layer->surface, &buffers, "Vibrance filter")) {
    ...
    status = ocularVibranceFilter(buffers.rgb_input, buffers.rgb_output,
                                  buffers.width, buffers.height, buffers.stride, vibrance);

    if (status != OC_STATUS_OK) {
        debug_log("WRN", "Vibrance filter: Ocular filter returned error %d", status);
        filter_utils_free_rgb_buffers(&buffers);
        return FALSE;
    }

    /* Convert back from RGB to Cairo ARGB32 */
    if (!filter_utils_rgb_to_cairo(layer->surface, &buffers, "Vibrance filter")) {
        filter_utils_free_rgb_buffers(&buffers);
        return FALSE;
    }
