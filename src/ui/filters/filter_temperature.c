...
gboolean filter_temperature_apply(ImageLayer *layer, const gfloat *values, gint num_values)
{
    ...
    if (!rgb_input || !rgb_output) {
        g_warning("Color temperature filter: Failed to allocate memory");
        g_free(rgb_input);
        g_free(rgb_output);
        return FALSE;
    }
    ...
    if (!adjustments_cairo_to_rgb(surface, rgb_input)) {
        g_warning("Color temperature filter: Failed to convert surface to RGB");
        g_free(rgb_input);
        g_free(rgb_output);
        return FALSE;
    }
    ...
    status = ocularColorTemperature(rgb_input, rgb_output, width, height, width * 3,
    ...
    if (!adjustments_rgb_to_cairo(surface, rgb_output)) {
        g_warning("Color temperature filter: Failed to convert RGB to surface");
        g_free(rgb_input);
        g_free(rgb_output);
        return FALSE;
    }
