...
gboolean filter_laplacian_edge_apply(ImageLayer* layer) {
    ...
    if (!rgb_input || !grayscale_input || !edge_output) {
        g_warning("Laplacian edge filter: Failed to allocate memory");
        g_free(rgb_input);
    ...
    if (!adjustments_cairo_to_rgb(surface, rgb_input)) {
        g_warning("Laplacian edge filter: Failed to convert surface to RGB");
        g_free(rgb_input);
        g_free(grayscale_input);
        g_free(edge_output);
        return FALSE;
    ...
    if (status != OC_STATUS_OK) {
        g_warning("Laplacian edge filter: Grayscale conversion returned error %d", status);
        g_free(rgb_input);
        g_free(grayscale_input);
        g_free(edge_output);
        return FALSE;
    ...
    if (status != OC_STATUS_OK) {
        g_warning("Laplacian edge filter: Ocular filter returned error %d", status);
        g_free(rgb_input);
        g_free(grayscale_input);
        g_free(edge_output);
        return FALSE;
    ...
    if (!adjustments_grayscale_to_cairo(surface, edge_output)) {
        g_warning("Laplacian edge filter: Failed to convert grayscale to surface");
        g_free(rgb_input);
        g_free(grayscale_input);
        g_free(edge_output);
        return FALSE;
