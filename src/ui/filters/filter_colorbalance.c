                                    ...
                                    OcToneBalanceMode mode, gboolean preserve_luminosity)
{
    ...
    if (!rgb_input || !rgb_output) {
        debug_log("WRN", "Color balance filter: Failed to allocate memory");
        g_free(rgb_input);
        g_free(rgb_output);
        return FALSE;
    }
    ...
    if (!adjustments_cairo_to_rgb(surface, rgb_input)) {
        debug_log("WRN", "Color balance filter: Failed to convert surface to RGB");
        g_free(rgb_input);
        g_free(rgb_output);
        return FALSE;
    }
    ...
    if (status != OC_STATUS_OK) {
        debug_log("WRN", "Color balance filter: Ocular filter returned error %d", status);
        g_free(rgb_input);
        g_free(rgb_output);
        return FALSE;
    }
    ...
    if (!adjustments_rgb_to_cairo(surface, rgb_output)) {
        debug_log("WRN", "Color balance filter: Failed to convert RGB to surface");
        g_free(rgb_input);
        g_free(rgb_output);
        return FALSE;
    }
