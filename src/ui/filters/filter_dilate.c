...
gboolean filter_dilate_apply(ImageLayer* layer, const gfloat* values, gint num_values) {
    ...
    if (!filter_utils_allocate_rgb_buffers(layer->surface, &buffers, "Dilate filter")) {
        return FALSE;
    }

    /* Convert from Cairo ARGB32 to RGB */
    if (!filter_utils_cairo_to_rgb(layer->surface, &buffers, "Dilate filter")) {
        filter_utils_free_rgb_buffers(&buffers);
        return FALSE;
    }

    /* Apply dilate filter using Ocular library
       Input and output: RGB format (stride = width * 3) */
    status = ocularDilateFilter(buffers.rgb_input, buffers.rgb_output,
                                buffers.width, buffers.height, buffers.stride, radius);

    if (status != OC_STATUS_OK) {
        debug_log("WRN", "Dilate filter: Ocular filter returned error %d", status);
        filter_utils_free_rgb_buffers(&buffers);
        return FALSE;
    }

    /* Convert back from RGB to Cairo ARGB32 */
    if (!filter_utils_rgb_to_cairo(layer->surface, &buffers, "Dilate filter")) {
        filter_utils_free_rgb_buffers(&buffers);
        return FALSE;
    }
