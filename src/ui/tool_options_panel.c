...
void tool_options_panel_sync_crop_from_rect(GtkWidget* drawing_area, gint w, gint h) {
    ...
    if (ratio_w_spin)
        vertical_spin_button_set_value(VERTICAL_SPIN_BUTTON(ratio_w_spin), (gdouble)rw);
    if (ratio_h_spin)
        vertical_spin_button_set_value(VERTICAL_SPIN_BUTTON(ratio_h_spin), (gdouble)rh);
    if (width_spin)
        vertical_spin_button_set_value(VERTICAL_SPIN_BUTTON(width_spin), (gdouble)w);
    if (height_spin)
        vertical_spin_button_set_value(VERTICAL_SPIN_BUTTON(height_spin), (gdouble)h);
...
static void on_crop_ratio_width_changed(GtkWidget* spin, gpointer user_data) {
    ToolOptionsPanel* panel = (ToolOptionsPanel*)user_data;
    gint w = (gint)vertical_spin_button_get_value(VERTICAL_SPIN_BUTTON(spin));
    gint h;
    ToolOptions* opts = tool_options_get_for_tool(TOOL_CROP);
    if (!opts)
        return;
...
static void on_crop_ratio_height_changed(GtkWidget* spin, gpointer user_data) {
    ToolOptionsPanel* panel = (ToolOptionsPanel*)user_data;
    gint h = (gint)vertical_spin_button_get_value(VERTICAL_SPIN_BUTTON(spin));
    gint w;
    ToolOptions* opts = tool_options_get_for_tool(TOOL_CROP);
    if (!opts)
        return;
...
static void on_crop_width_changed(GtkWidget* spin, gpointer user_data) {
    ToolOptionsPanel* panel = (ToolOptionsPanel*)user_data;
    ...
    if (tool_options_get_crop_link(opts) && opts->crop_ratio_w > 0 && opts->crop_ratio_h > 0 && panel->crop_panel) {
        ...
        if (height_spin) {
            g_crop_spin_updating = TRUE;
            vertical_spin_button_set_value(VERTICAL_SPIN_BUTTON(height_spin), (gdouble)new_h);
...
static void on_crop_height_changed(GtkWidget* spin, gpointer user_data) {
    ToolOptionsPanel* panel = (ToolOptionsPanel*)user_data;
    if (g_crop_spin_updating)
        return;
    gint h = (gint)vertical_spin_button_get_value(VERTICAL_SPIN_BUTTON(spin));
    ...
    if (tool_options_get_crop_link(opts) && opts->crop_ratio_w > 0 && opts->crop_ratio_h > 0 && panel->crop_panel) {
        ...
        if (width_spin) {
            g_crop_spin_updating = TRUE;
            vertical_spin_button_set_value(VERTICAL_SPIN_BUTTON(width_spin), (gdouble)new_w);
...
ToolOptionsPanel* create_tool_options_panel(void) {
    ...
    tool_opts_panel->brush_panel = load_panel_from_glade(
        "/ui/tools/brush_options.glade", "brush_options_panel",
        &brush_title,
        &brush_size,
        &brush_opacity,
        &brush_hardness,
    ...
    tool_opts_panel->eraser_panel = load_panel_from_glade(
        "/ui/tools/eraser_options.glade", "eraser_options_panel",
        &eraser_title,
        &eraser_size,
        &eraser_opacity,
        &eraser_hardness,
    ...
    if (gtk_builder_add_from_resource(pencil_builder, "/ui/tools/pencil_options.glade", &pencil_error)) {
    ...
    if (gtk_builder_add_from_resource(paintbucket_builder, "/ui/tools/paintbucket_options.glade", &paintbucket_error)) {
    ...
    if (gtk_builder_add_from_resource(color_picker_builder, "/ui/tools/color_picker_options.glade", &color_picker_error)) {
    ...
    if (gtk_builder_add_from_resource(rect_select_builder, "/ui/tools/rect_select_options.glade", &rect_select_error)) {
    ...
    if (gtk_builder_add_from_resource(ellipse_select_builder, "/ui/tools/elliptical_select_options.glade", &ellipse_select_error)) {
    ...
    if (gtk_builder_add_from_resource(polygon_select_builder, "/ui/tools/polygon_select_options.glade", &polygon_select_error)) {
    ...
    if (gtk_builder_add_from_resource(lasso_select_builder, "/ui/tools/lasso_select_options.glade", &lasso_select_error)) {
    ...
    if (gtk_builder_add_from_resource(magic_wand_builder, "/ui/tools/magic_wand_select_options.glade", &magic_wand_error)) {
    ...
    if (gtk_builder_add_from_resource(crop_builder, "/ui/tools/crop_options.glade", &crop_error)) {
        ...
        if (tool_opts_panel->crop_panel) {
            ...
            crop_ratio_w_spin = ui_utils_replace_spin_with_vertical(crop_ratio_w_spin);
            crop_ratio_h_spin = ui_utils_replace_spin_with_vertical(crop_ratio_h_spin);
            crop_width_spin = ui_utils_replace_spin_with_vertical(crop_width_spin);
            crop_height_spin = ui_utils_replace_spin_with_vertical(crop_height_spin);
            ...
            if (crop_ratio_w_spin) {
                ...
                if (crop_opts)
                    vertical_spin_button_set_value(VERTICAL_SPIN_BUTTON(crop_ratio_w_spin), (gdouble)crop_opts->crop_ratio_w);
            }
            if (crop_ratio_h_spin) {
                ...
                if (crop_opts)
                    vertical_spin_button_set_value(VERTICAL_SPIN_BUTTON(crop_ratio_h_spin), (gdouble)crop_opts->crop_ratio_h);
            }
            if (crop_width_spin) {
                g_signal_connect(crop_width_spin, "value-changed", G_CALLBACK(on_crop_width_changed), tool_opts_panel);
                if (crop_opts)
                    vertical_spin_button_set_value(VERTICAL_SPIN_BUTTON(crop_width_spin), (gdouble)crop_opts->crop_width);
            }
            if (crop_height_spin) {
                g_signal_connect(crop_height_spin, "value-changed", G_CALLBACK(on_crop_height_changed), tool_opts_panel);
                if (crop_opts)
                    vertical_spin_button_set_value(VERTICAL_SPIN_BUTTON(crop_height_spin), (gdouble)crop_opts->crop_height);
            }
            if (crop_link_toggle) {
    ...
    if (gtk_builder_add_from_resource(move_builder, "/ui/tools/move_options.glade", &move_error)) {
    ...
    /* ── Load text tool options panel from Glade ── */
    {
        ...
        ui_utils_builder_set_translation_domain(builder);
        GError* err = NULL;
        if (gtk_builder_add_from_resource(builder, "/ui/tools/text_options.glade", &err)) {
    ...
    /* ── Load gradient tool options panel from Glade ── */
    {
        ...
        ui_utils_builder_set_translation_domain(builder);
        GError* err = NULL;
        if (gtk_builder_add_from_resource(builder, "/ui/tools/gradient_options.glade", &err)) {
